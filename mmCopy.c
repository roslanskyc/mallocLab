...
#define BSIZE       32      /* minimum block size (header + footer + payload) */
...
static void place(void *bp, size_t asize) {
    ...
    PUT(PSUB(newh,8), PACK(asize, 1));

    PUT(newh, PACK((size_t)PSUB(osize, (char *)asize), 0));
    PUT(foot, PACK((size_t)PSUB(osize, (char *)asize), 0));
